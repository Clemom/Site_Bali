...
document.querySelectorAll("button").forEach((button) => {
  button.addEventListener("click", function () {
    document
      ...
      .forEach((b) => b.classList.remove("active-button"));
...
function chargerParcours(duree) {
  ...
  if (duree === "25jours") {
    jsonParcours =
      "https://raw.githubusercontent.com/Clemom/Site_Bali/main/Parcours_Ganesh/Parcours_Ganesh25.json";
  } else if (duree === "20jours") {
    jsonParcours =
      "https://raw.githubusercontent.com/Clemom/Site_Bali/main/Parcours_Ganesh/Parcours_Ganesh20.json";
  } else {
    jsonParcours =
      "https://raw.githubusercontent.com/Clemom/Site_Bali/main/Parcours_Ganesh/Parcours_Ganesh15.json";
  }
    ...
    .then((data) => {
      ...
      if (duree === "25jours") {
        filtreParcours = data.parcours.filter(
          (parcours) =>
            parcours.duree === "15jours" ||
            parcours.duree === "20jours" ||
            parcours.duree === "25jours"
        );
      } else if (duree === "20jours") {
        filtreParcours = data.parcours.filter(
          (parcours) =>
            parcours.duree === "15jours" || parcours.duree === "20jours"
        );
      } else {
        filtreParcours = data.parcours.filter(
          (parcours) => parcours.duree === "15jours"
        );
      }
